...
app = Flask(__name__, static_url_path='',
            static_folder=os.path.join(APP_ROOT, "dist"))
CORS(app)
...
def get_client(model_config):
    ...
    client = Client(
        ...
        html.escape(model_config["api_base"]
                    ) if "api_base" in model_config else None,
...
def get_example_dataset_list():
    ...
    for dataset in example_datasets:
        ...
        try:
            info_obj = {'name': name, 'challenges': challenges,
...
def test_model():
    ...
    if request.is_json:
        ...
        try:
            ...
            logger.info(
                f"welcome message: {response.choices[0].message.content}")
...
def process_data_on_load_request():
    ...
    if request.is_json:
        ...
        response = flask.jsonify(
            {"status": "ok", "token": token, "result": candidates})
    else:
        response = flask.jsonify(
            {"token": -1, "status": "error", "result": []})
...
def clean_data_request():
    ...
    if request.is_json:
        ...
        candidates = agent.run(
            content['content_type'], content["raw_data"], content["image_cleaning_instruction"])
...
def derive_data():
    ...
    if request.is_json:
        ...
        if mode == "recommendation":
            ...
            results = agent.run(input_tables, instruction, [
                                field['name'] for field in new_fields], prev_messages)
        ...
        while results[0]['status'] == 'error' and repair_attempts < max_repair_attempts:
            ...
            if mode == "transform":
                results = agent.followup(input_tables, prev_dialog, [
                                         field['name'] for field in new_fields], new_instruction)
            if mode == "recommendation":
                results = agent.followup(
                    input_tables, prev_dialog, new_instruction)

            repair_attempts += 1

        response = flask.jsonify(
            {"token": token, "status": "ok", "results": results})
    else:
        response = flask.jsonify(
            {"token": "", "status": "error", "results": []})
...
def refine_data():
    ...
    if request.is_json:
        ...
        agent = DataTransformationAgentV2(client=client)
        results = agent.followup(input_tables, dialog, [
                                 field['name'] for field in output_fields], new_instruction)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-p", "--port", type=int, default=5000,
                        help="The port number you want to use")
