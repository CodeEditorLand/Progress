...
def table_hash(table):
    ...
    frozen_table = tuple(sorted(
        [tuple([hash(value_handling_func(r[key])) for key in schema]) for r in table]))
...
def extract_code_from_gpt_response(code_raw, language):
    ...
    prefix_pos = [m.span()[0]
                  for m in re.compile(f"```{language}").finditer(code_raw)]
...
def get_field_summary(field_name, df, field_sample_size):
    try:
        values = sorted(
            [x for x in list(set(df[field_name].values)) if x != None])
    except:
    ...
    if len(values) <= sample_size:
        ...
        val_sample = values[:int(sample_size / 2)] + ["..."] + \
            values[-(sample_size - int(sample_size / 2)):]

    val_str = ', '.join([str(s) if ',' not in str(
        s) else f'"{str(s)}"' for s in val_sample])
...
def generate_data_summary(input_tables, include_data_samples=True, field_sample_size=7):
    ...
    input_table_names = [
        f'{string_to_py_varname(t["name"])}' for t in input_tables]
    ...
    for input_data in input_tables:
        ...
        s = '\n\t'.join([get_field_summary(fname, df, field_sample_size)
                        for fname in list(df.columns.values)])
        field_summaries.append(s)

    table_field_summaries = [
        f'table_{i} ({input_table_names[i]}) fields:\n\t{s}' for i, s in enumerate(field_summaries)]

    if include_data_samples:
        table_sample_strings = [
            f'table_{i} ({input_table_names[i]}) sample:\n\n```\n{pd.DataFrame(data_sample).to_csv(sep="|")}......\n```' for i, data_sample in enumerate(data_samples)]
    else:
        table_sample_strings = [
            '' for i, data_sample in enumerate(data_samples)]

    table_summary = "\n\n".join([f'{field_summary}\n\n{sample_str}' for field_summary, sample_str in zip(
        table_field_summaries, table_sample_strings)])
