...
- **Configure environment variable (optional)s**
    ...
    - required fields for different providers are different, please refer to the
      ...
      more details.
        - currently only endpoint, model, api_key, api_base, api_version are
          supported.
    - this helps data formulator to automatically load the API keys when you run
...
- **Development mode**
    ...
    Run the front-end in development mode using, allowing real-time edits and
    previews:
...
- **Build the frontend and then the backend**
    ...
    This builds the app for production to the `py-src/data_formulator/dist`
    folder.
    ...
    This will create a python wheel in the `dist/` folder. The name would be
    `data_formulator-<version>-py3-none-any.whl`

- **Test the artifact**

    You can then install the build result wheel (testing in a virtual
    ...
    Open [http://localhost:5000](http://localhost:5000) to view it in the
    browser.
