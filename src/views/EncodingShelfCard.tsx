	...
	if (activeFields.length == 0) {
		...
		let activeBaseFields = conceptShelfItems.filter((field) => {
			return activeFields
				.map((f) =>
					f.source == "derived"
						? findBaseFields(f, conceptShelfItems).map(
								(f2) => f2.id,
							)
						: [f.id],
				)
		...
		let activeOriginalFields = activeBaseFields.filter(
			(field) => field.source == "original",
		);

		if (
			activeOriginalFields.length == 0 &&
		...
		let tablesToAdd = tables.filter((t) =>
			activeOriginalFields
				.map((f) => f.tableRef as string)
				.includes(t.id),
		);
...
export const TriggerCard: FC<{
	...
	hideFields?: boolean;
	label?: string;
}> = function ({ label, className, trigger, hideFields }) {
	...
	return (
		<Box sx={{}}>
			...
			<Card
				...
				sx={{
					cursor: "pointer",
					backgroundColor: "rgba(255, 160, 122, 0.07)",
					"&:hover": {
				...
				<Stack
					direction="row"
					sx={{ marginLeft: 1, marginRight: "auto", fontSize: 12 }}
					alignItems="center"
					gap={"2px"}>
...
export const MiniTriggerCard: FC<{
	className?: string;
	trigger: Trigger;
	hideFields?: boolean;
	label?: string;
	...
	return (
		<Box sx={{}}>
			<Card
				...
				<Stack
					direction="row"
					sx={{ marginRight: "auto", fontSize: 11 }}
					alignItems="center"
					gap={"2px"}>
	...
	return (
		<Box
			...
			<Menu anchorEl={anchorEl} open={open} onClose={handleClose}>
				{tables.map((table) => {
					const isSelected = !!actionTableIds.find(
						(t) => t === table.id,
					);
					return (
						<MenuItem
...
export const EncodingShelfCard: FC<EncodingShelfCardProps> = function ({
	...
	let existMultiplePossibleBaseTables =
		tables.filter((t) => t.derive == undefined).length > 1;
	...
	let handleUpdateChartType = (newChartType: string) => {
		dispatch(
			dfActions.updateChartType({ chartId, chartType: newChartType }),
		);
	};
		...
		.map(([group, channelList]) => {
			let component = (
				<Box>
					...
					{channelList
						.filter((channel) =>
							Object.keys(encodingMap).includes(channel),
						)
						.map((channel) => (
							<EncodingBox
								key={`shelf-${channel}`}
	...
	let activeCustomFields = activeBaseFields.filter(
	...
	let existsWorkingTable =
		activeBaseFields.length == 0 ||
		activeBaseFields.every((f) => currentTable.names.includes(f.name));
	...
	let deriveNewData = (overrideTableId?: string) => {
		...
		if (
			...
			dispatch(
				dfActions.updateTableRef({
					chartId: chartId,
					tableRef: tempTable.id,
				}),
			);

			//dispatch(dfActions.resetDerivedTables([])); //([{code: "", data: inputData.rows}]));
			dispatch(
				dfActions.changeChartRunningStatus({ chartId, status: true }),
			);
		...
		if (mode == "formulate" && currentTable.derive?.dialog) {
			...
			if (
				...
				messageBody = JSON.stringify({
					...
					output_fields: activeBaseFields.map((f) => {
						return { name: f.name };
					}),
				...
				if (data.results.length > 0) {
					if (data["token"] == token) {
						...
						if (candidates.length == 0) {
							...
							if (overrideTableId) {
								dispatch(
									dfActions.overrideDerivedTables(
										candidateTable,
									),
								);
							} else {
							...
							if (
								...
								if (
									Object.values(chartToOverride.encodingMap)
										.map((enc) => enc.fieldID)
										.filter(
											(fid) =>
												fid != undefined &&
					...
					<Tooltip
						title={
							<Typography sx={{ fontSize: 11 }}>
								...
								<TableRowsIcon
									sx={{ fontSize: 10, marginBottom: "-1px" }}
								/>
			...
			<Box key="mark-selector-box" sx={{ flex: "0 0 auto" }}>
				<FormControl
					...
					<Select
						...
						onChange={(event) => {
							handleUpdateChartType(event.target.value);
						}}>
						{Object.entries(CHART_TEMPLATES).map(
							([group, templates]) => {
								return [
									<ListSubheader
										sx={{
