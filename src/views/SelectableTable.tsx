...
export const SelectableTableBody: React.FC<SelectableTableBodyProps> = ({
	rows,
	columnDefs,
	onSelect,
	selectedBounds,
...
export const SelectableTable: React.FC<SelectableTableProps> = ({
	...
	const onClickCell = (event: any) => {
		...
		if (item) {
			...
			$tableRef.current?.props.onSelectionFinish!([
			...
			<TableContainer sx={{ height: "100%" }}>
				...
				<SelectableGroup
					...
					onSelectionFinish={(selected: any[]) => {
						...
						let left = _.min(
								selected.map((x) => x.props.indices[0]),
							),
							right = _.max(
								selected.map((x) => x.props.indices[0]),
							),
							bottom = _.max(
								selected.map((x) => x.props.indices[1]),
							),
							top = _.min(
								selected.map((x) => x.props.indices[1]),
							);
