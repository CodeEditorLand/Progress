...
const SimpleDataHeaderFC: FC<SimpleDataHeaderProps> = (
	props: SimpleDataHeaderProps,
) => {
	...
	return (
		<Box className="data-view-header-container">
			<Box className="data-view-header-content">
				<Typography
					align="center"
					className="data-view-header-label"
					sx={{ fontSize: 10 }}>
					{props.displayName}
				</Typography>
				{sort}
...
export const SimpleDataViewFC: FC<SimpleDataViewProps> =
	function SimpleDataView({ table, highlightedFields, height = 300 }) {
		let $gridApi = undefined;

		const dispatch = useDispatch();

		const rowData = table
			? table.rows.map((r: any, i: number) => ({ ...r }))
			: [];
		const columns = table
			? table.names.map((name: string, index: number) => {
					return {
						...
						headerClass: highlightedFields.includes(name)
							? "highlighted-column-header"
							: "",
						cellStyle: function (params: any) {
							if (
								highlightedFields.includes(params.colDef.field)
							) {
								return {
				...
				<Box sx={{ display: "flex" }}>
					<Button
						...
						<SubdirectoryArrowRightIcon
							...
							sx={{ marginRight: "4px" }}
						/>
						continue analysis from this table
					</Button>
				</Box>
				<Box
					className="table-container table-container-small ag-theme-material"
					sx={{ fontSize: "10px", height: height }}>
					<AgGridReact
