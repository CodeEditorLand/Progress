...
export const InfoPanelFC: FC<{
	$tableRef: React.RefObject<AgGridReact | SelectableGroup>;
}> = function InfoPanel({ $tableRef }) {
	// reference to states
	...
	let tabEntries = [
		{
			label: "Selected Data",
			panelElement: dataSelectionView,
			highlight: false,
		},
	];
		...
		<Box
			...
			<Box
				...
				<IconButton
					...
					{hidePanel ? <UnfoldMoreIcon /> : <UnfoldLessIcon />}
				</IconButton>
				<Tabs value={Math.abs(tabValue)}>
					{tabEntries.map((entry, index) => (
						<Tab
							...
							className={
								entry.highlight && index != tabValue
									? "background-highlight"
									: ""
							}
							onClick={(event) => {
								handleTabSwitch(event, index);
							}}
							label={entry.label}
							{...a11yProps(index)}
							sx={{
