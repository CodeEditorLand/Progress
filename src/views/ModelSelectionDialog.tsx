...
import { DataFormulatorState, dfActions, ModelConfig } from "../app/dfSlice";
...
export const ModelSelectionButton: React.FC<{}> = ({}) => {
	...
	const [isLoadingModelOptions, setIsLoadingModelOptions] =
	...
	let updateModelStatus = (
		...
		dispatch(
			dfActions.updateModelStatus({ id: model.id, status, message }),
		);
	};
	...
	const [newApiVersion, setNewApiVersion] = useState<string | undefined>(
		undefined,
	);
	...
	let newModelEntry = (
		<TableRow
			...
			<TableCell align="right">
				<Radio
					checked={tempSelectedModelId == undefined}
					name="radio-buttons"
					inputProps={{ "aria-label": "Select this model" }}
				/>
			</TableCell>
			...
			<TableCell align="left">
				<Autocomplete
					freeSolo
					onChange={(event: any, newValue: string | null) => {
						setNewModel(newValue || "");
					}}
					value={newModel}
					options={
						newEndpoint && providerModelOptions[newEndpoint]
							? providerModelOptions[newEndpoint]
							: []
					}
			...
			<Table
				...
				<TableHead>
					<TableRow>
						...
						<TableCell sx={{ fontWeight: "bold", width: "240px" }}>
							api_key
						</TableCell>
						<TableCell
				...
				<TableBody>
					{models.map((model) => {
						...
						if (status == "unknown") {
							message =
								"Click the status icon to test again before applying.";
						} else if (status == "error") {
							message =
								testedModels.find((t) => t.id == model.id)
									?.message || "Unknown error";
						}

						const borderStyle = ["error", "unknown"].includes(
							status,
						)
							? "1px dashed text.secondary"
							: undefined;
						const noBorderStyle = ["error", "unknown"].includes(
							status,
						)
							? "none"
							: undefined;
						...
						<TableCell
							...
							<Typography>
								...
								<a
									...
									check out supported endpoint / models here
								</a>
								. If using custom providers that are compatible
								with the OpenAI API, choose 'openai' as the
								provider.
							</Typography>
							<Typography>
								Models with limited code generation capabilities
								(e.g., llama3.2) may fail frequently to derive
								new data.
							</Typography>
						</TableCell>
					</TableRow>
		...
		<>
			<Tooltip title="select model">
				<Button
					...
					{selectedModelId
						? `Model: ${(models.find((m) => m.id == selectedModelId) as any)?.model}`
						: "Select A Model"}
				</Button>
			</Tooltip>
			<Dialog
				...
				<DialogContent>{modelTable}</DialogContent>
				<DialogActions>
					...
					<Button
						disabled={getStatus(tempSelectedModelId) !== "ok"}
						variant={
							selectedModelId == tempSelectedModelId
								? "text"
								: "contained"
						}
						onClick={() => {
							dispatch(
								dfActions.selectModel(tempSelectedModelId),
							);
							setModelDialogOpen(false);
						}}>
						apply model
