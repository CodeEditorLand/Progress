...
export const FreeDataViewFC: FC<FreeDataViewProps> = function DataView({
	...
	let renderTableBody = (targetTable: DictTable | undefined) => {
		const rowData = targetTable
			? targetTable.rows.map((r: any, i: number) => ({
					...r,
					"#rowId": i,
				}))
			: [];
		let colDefs = targetTable
			? targetTable.names.map((name, i) => {
					return {
						...
						source:
							conceptShelfItems.find((f) => f.name == name)
								?.source || "original",
					};
				})
			: [];
		...
		return (
			<SelectableDataGrid
				$tableRef={$tableRef}
				tableName={targetTable?.id || "table"}
				rows={rowData}
				columnDefs={colDefs}
	...
	return (
		<Box
			...
			<Box sx={{ display: "flex" }}>
				...
				<Typography
					sx={{ display: "flex", color: "darkgray" }}
					component="span">
