...
import { Channel, FieldItem } from "../components/ComponentType";
...
export const ConceptShelf: FC<ConceptShelfProps> = function ConceptShelf() {
	...
	let handleUpdateConcept = (field: FieldItem) =>
		dispatch(dfActions.updateConceptItems(field));
	...
	useEffect(() => {
		...
		if (focusedTable) {
			...
			let conceptIdsToDelete = conceptShelfItems
				.filter(
					(field) =>
						field.temporary == true &&
						!charts.some((c) =>
							Object.values(c.encodingMap).some(
								(enc) => enc.fieldID == field.id,
							),
						) &&
		...
		<Tooltip title="Create a new concept">
			<Button
				sx={{
					...
					"&:hover": {
						backgroundColor: alpha(theme.palette.custom.main, 0.1),
					},
					float: "right",
					textTransform: "none",
					minWidth: "20px",
				...
				onClick={() => {
					if (
						conceptShelfItems.filter((f) => f.name === "").length >
						0
					) {
						return;
					}
