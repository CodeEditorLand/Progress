...
let SingleThreadView: FC<{
	...
	if (leafTable.derive) {
		let firstNewTableIndex = triggers.findIndex(
			(tg) => !usedTableIds.includes(tg.tableId),
		);
		firstNewTableIndex =
			firstNewTableIndex == -1 ? triggers.length : firstNewTableIndex;
		triggers =
			firstNewTableIndex > 0
				? triggers.slice(firstNewTableIndex - 1)
				: triggers;

		tableIdList = [
			...triggers.map((trigger) => trigger.tableId),
			leafTable.id,
		];

		triggerCards = triggers.map((trigger, i) => {
			let selectedClassName =
				trigger.chartRef == focusedChartId ? "selected-card" : "";

			let extractActiveFields = (t: Trigger) => {
				let encodingMap = (
					charts.find((c) => c.id == t.chartRef) as Chart
				).encodingMap;
			...
			let fieldsIdentical = _.isEqual(
				previousActiveFields,
				currentActiveFields,
			);
			...
			return (
				<Box
					...
					key={`trigger-card-${trigger.chartRef}`}>
					{triggerCard}
	...
	let threadIsFocused =
		focusedChart &&
		tableIdList.includes(focusedChart.tableRef) &&
		!usedTableIds.includes(focusedChart.tableRef);

	let tableList = tableIdList.map((tableId, i) => {
		// filter charts relavent to this
		let relevantCharts = chartElements.filter(
			(ce) => ce.tableId == tableId && !usedTableIds.includes(tableId),
		);
		let table = tables.find((t) => t.id == tableId);

		let selectedClassName =
			tableId == focusedTableId ? "selected-card" : "";
		...
		let releventChartElements = relevantCharts.map((ce, j) => (
			<Box
				...
				sx={{
					display: "flex",
					padding: 0,
					paddingBottom: j == relevantCharts.length - 1 ? 1 : 0.5,
					...collapsedProps,
				}}>
				...
				<Box
					...
					<Stack
						...
						sx={{
							fontSize: "12px",
							fontWeight:
								tableId == focusedTableId ? "bold" : "normal",
						}}
						alignItems="center"
						gap={"2px"}>
						<TableRowsIcon
						...
						/>
						<Typography
							sx={{ fontSize: "12px", fontWeight: "inherit" }}>
							{tableId}
						</Typography>
					</Stack>
				</Box>
				...
				<Card
					...
					onClick={() => {
						...
						if (focusedChart?.tableRef != tableId) {
							let firstRelatedChart =
								charts.find(
									(c: Chart) =>
										c.tableRef == tableId &&
										c.intermediate == undefined,
								) ||
								charts.find(
									(c: Chart) => c.tableRef == tableId,
								);
							if (firstRelatedChart) {
								if (
									firstRelatedChart.intermediate == undefined
								) {
									dispatch(
										dfActions.setFocusedChart(
											firstRelatedChart.id,
										),
									);
								}
							} else {
		...
		return [
			...
			i == tableIdList.length - 1 ? (
				...
				</Box> //<Divider  sx={{marginLeft: 1, width: "20px", borderColor: 'darkgray', borderStyle: 'dashed'}} orientation="horizontal" />
			) : (
				""
			),
		];
...
export const DataThread: FC<{}> = function ({}) {
	...
	let chartElements = charts
		.filter((chart) => !chart.intermediate)
		.map((chart, index) => {
			...
			if (assembledChart["data"]["values"].length > 5000) {
				...
				assembledChart["data"] = {
					"values": getRandomSubarray(values, 5000),
				};
			}
							...
							<Tooltip title="delete chart">
								<IconButton
									...
									onClick={(event) => {
										event.stopPropagation();
										dispatch(
											dfActions.deleteChartById(chart.id),
										);
									}}>
									<DeleteIcon fontSize="small" />
								</IconButton>
	...
	let leafTables = tables.filter(
		(t) => !tables.some((t2) => t2.derive?.trigger.tableId == t.id),
	);
	...
	let threadDrawerWidth = Math.max(
		Math.min(600, leafTables.length * 200),
		212,
	);

	let view = (
		<Box
			width={drawerOpen ? threadDrawerWidth + 12 : 224}
			sx={{
				overflowY: "auto",
				position: "relative",
				display: "flex",
			...
			{leafTables.map((lt, i) => {
				let usedTableIds = leafTables
					.slice(0, i)
					.map((x) => [
						x.id,
						...(getTriggers(x, tables).map((y) => y.tableId) || []),
					])
					.flat();
				return (
					<SingleThreadView
	...
	let jumpButtonsDrawerOpen = (
		<ButtonGroup size="small" color="primary">
			{_.chunk(
				Array.from({ length: leafTables.length }, (_, i) => i),
				3,
			).map((group, groupIdx) => {
	...
	let jumpButtons = drawerOpen
		? jumpButtonsDrawerOpen
		: jumpButtonDrawerClosed;
			...
			<Box
				...
				<Tooltip title={drawerOpen ? "collapse" : "expand"}>
					<IconButton
						...
						onClick={() => {
							setThreadDrawerOpen(!threadDrawerOpen);
						}}>
						{drawerOpen ? (
							<ChevronLeftIcon />
						) : (
	...
	return <Box sx={{ display: "flex", flexDirection: "row" }}>{carousel}</Box>;
};
