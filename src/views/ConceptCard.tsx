...
export const ConceptCard: FC<ConceptCardProps> = function ConceptCard({
	field,
}) {
	...
	let focusedChartRefTable = tables.find(
		(t) => t.id == focusedChart?.tableRef,
	);

	const [editMode, setEditMode] = useState(field.name == "" ? true : false);

	const dispatch = useDispatch();
	let handleDeleteConcept = (conceptID: string) =>
		dispatch(dfActions.deleteConceptItemByID(conceptID));
	let handleUpdateConcept = (concept: FieldItem) =>
		dispatch(dfActions.updateConceptItems(concept));
	...
	if (field.source == "derived") {
		let parentConceptNames = (
			field.transform as ConceptTransformation
		).parentIDs
			.map(
				(parentID) =>
					conceptShelfItems.find(
						(c) => c.id == parentID,
					) as FieldItem,
			)
			.map((f) => f.name);
		notInFocusedTable = parentConceptNames.some(
			(name) => !focusedChartRefTable?.names.includes(name),
		);
	} else {
	...
	let editOption =
		field.source === "original" ? undefined : (
	...
	let deriveOption = (
		<Tooltip key="derive-icon-button" title="derive new concept">
			<IconButton
				...
				color="primary"
				aria-label="derive new concept"
				component="span"
				onClick={() => {
					if (
						conceptShelfItems.filter(
							(f) =>
								f.source == "derived" &&
								f.name == "" &&
	...
	let typeIconMenu = (
		<div>
			<Tooltip title={`${field.type} type`}>
				<IconButton
					size="small"
					sx={{ fontSize: "inherit", padding: "2px" }}
					color="primary"
			...
			<Menu
				...
				{TypeList.map((t, i) => (
					<MenuItem
						...
						<Typography
							component="span"
							sx={{ fontSize: "inherit", marginLeft: "8px" }}>
							{t}
						</Typography>
					</MenuItem>
				))}
	...
	let boxShadow = editMode
		? "0 2px 4px 0 rgb(0 0 0 / 20%), 0 2px 4px 0 rgb(0 0 0 / 19%)"
		: "";

	let cardComponent = (
		<Card
			sx={{ minWidth: 60, backgroundColor }}
			variant="outlined"
			style={{
				opacity,
				border,
				boxShadow,
				fontStyle,
				marginLeft: field.source == "derived" ? "10px" : "3px",
			}}
			color="secondary"
			...
			<Box
				...
				<Typography
					...
					sx={{ fontSize: 13, height: 28, width: "100%" }}
					component={"span"}
					gutterBottom>
					{typeIconMenu}
					{fieldNameEntry}
				...
				<Box
					className="draggable-card-action-button"
					sx={{
						position: "absolute",
						right: 1,
		...
		<Box>
			...
			<ButtonGroup
				...
				sx={{
					...
					"& button": { textTransform: "none", padding: "2px 4px" },
					flexGrow: 1,
...
export const CustomConceptForm: FC<ConceptFormProps> =
	function CustomConceptForm({
		...
		const [dtype, setDtype] = useState(
			concept.name == "" ? "auto" : (concept.type as string),
		);
		const handleDtypeChange = (event: SelectChangeEvent) => {
		...
		let nameField = (
			<TextField
				...
				label="concept name"
				value={name}
				sx={{ minWidth: 120, maxWidth: 160, flex: 1 }}
				FormHelperTextProps={{
					style: { fontSize: 8, marginTop: 0, marginLeft: "auto" },
				}}
				multiline
				helperText={
					conceptShelfItems.some(
						(f) => f.name == name && f.id != concept.id,
					)
				...
				<Select
					...
					{typeList.map((t, i) => (
						<MenuItem value={t} key={`${concept.id}-${i}`}>
							<Typography
								component="span"
								sx={{ fontSize: "inherit", marginLeft: "0px" }}>
								{t}
							</Typography>
						</MenuItem>
					))}
		...
		while (true) {
			let newChildrens = conceptShelfItems
				.filter(
					(f) =>
						f.source == "derived" &&
						!childrenConceptIDs.includes(f.id) &&
						f.transform?.parentIDs.some((pid) =>
							childrenConceptIDs.includes(pid),
						),
				)
				.map((f) => f.id);
		...
		cardTopComponents = [nameField, typeField];
		...
		if (
			name == "" ||
			conceptShelfItems.some((f) => f.name == name && f.id != concept.id)
		) {
			...
			<Box sx={{ display: "flex", flexDirection: "column" }}>
				<Box
					...
					sx={{
						display: "flex",
						flexWrap: "wrap",
						"& > :not(style)": { margin: "4px" /*width: '25ch'*/ },
					}}
					noValidate
					autoComplete="off">
					<Box
					...
					<ButtonGroup
						...
						<IconButton
							...
							disabled={
								conceptShelfItems.filter(
									(f) =>
										f.source == "derived" &&
										f.transform?.parentIDs.includes(
											concept.id,
										),
								).length > 0
							}
						...
						<Button
							size="small"
							variant={
								checkCustomConceptDiff()
									? "contained"
									: "outlined"
							}
							disabled={
								saveDisabledMsg.length > 0 ||
								checkCustomConceptDiff() == false
							}
							onClick={() => {
...
export const DerivedConceptForm: FC<ConceptFormProps> =
	function DerivedConceptForm({
		concept,
		handleUpdateConcept,
		handleDeleteConcept,
		turnOffEditMode,
		...
		const [transformCode, setTransformCode] =
			useState<string>(formattedCode);
		const [transformDesc, setTransformDesc] = useState<string>(
			conceptTransform.description || "",
		);
		const [transformParentIDs, setTransformParentIDs] = useState<string[]>(
			conceptTransform.parentIDs || [],
		);
		...
		const [collapseVisInspector, setCollapseVisInspector] =
		...
		useEffect(() => {
			if (concept.source == "derived") {
				let result = deriveTransformExamplesV2(
					transformCode,
					transformParentIDs,
					NUM_SAVED_TRANSFORM_RESULTS,
					conceptShelfItems,
					tables,
				);
		...
		const [codeGenInProgress, setCodeGenInProgress] =
		...
		let nameField = (
			<TextField
				...
				label="concept name"
				value={name}
				sx={{ minWidth: 120, flex: 1, paddingBottom: 1 }}
				FormHelperTextProps={{
					style: { fontSize: 8, marginTop: 0, marginLeft: "auto" },
				}}
				multiline
		...
		let affiliatedTableId = conceptShelfItems.find(
			(f) => f.id == conceptTransform.parentIDs[0],
		)?.tableRef;
			...
			<FormControl
				...
				<Select
					...
					renderValue={(selected) => (
						<Typography
							key={selected[0]}
							sx={{ whiteSpace: "normal", fontSize: "inherit" }}>
					...
					onChange={(
						event: SelectChangeEvent<typeof transformParentIDs>,
					) => {
						const {
							target: { value },
						} = event;
					...
					{conceptShelfItems
						.filter(
							(t) =>
								t.name != "" && t.tableRef == affiliatedTableId,
						)
						.map((t, i) => (
							<MenuItem
								value={t.id}
								key={`${concept.id}-${t.id}`}
								sx={{ fontSize: 12, marginLeft: "0px" }}
								disabled={childrenConceptIDs.includes(t.id)}>
								{
									<Checkbox
										sx={{ padding: 0.5 }}
										size="small"
										checked={
											transformParentIDs.indexOf(t.id) >
											-1
										}
									/>
								}
		...
		let parentConcepts = transformParentIDs.map(
			(parentID) => conceptShelfItems.filter((c) => c.id == parentID)[0],
		);
		...
		if (transformCode && transformResult.length > 0) {
			let colNames: [string[], string] = [
				parentConcepts.map((f) => f.name),
				name,
			];
			...
			viewExamples = (
				<Box
					...
					<InputLabel shrink>
						illustration of the generated function
					</InputLabel>
		...
		let codeArea = (
			<Box
				...
				sx={{
					...
					position: "relative",
					marginTop: "5px",
					minHeight: codeGenInProgress ? "10px" : "0px",
				}}>
				{codeGenInProgress ? (
					<Box
						sx={{
		...
		let handleProcessResults = (
			status: string,
			rawCodeList: string[],
		): string[] => {
			setCodeGenInProgress(false);
			if (status == "ok") {
				let candidates = processCodeCandidates(
					rawCodeList,
					transformParentIDs,
					conceptShelfItems,
					tables,
				...
				dispatch(
					dfActions.addMessages({
						...
						"type": "error",
						"value":
							"unable to generate the desired transformation, please try again.",
					}),
				);
				return [];
			}
		...
		let inputFieldsInfo = parentConcepts.map((c) => {
			...
			if (c.source == "derived") {
				...
				values = deriveTransformExamplesV2(
					transform.code,
					transform.parentIDs,
					5,
					conceptShelfItems,
				...
				values = getDomains(c, tables)
					.map((d) => d.slice(0, 5))
					.flat(); //c.domain.values.slice(0, 5);
			}
			return {
				name: c.name,
				type:
					values.length > 0
						? typeof values[Math.floor((values.length - 1) / 2)]
						: "string",
				values,
			};
		});
		...
		let inputDataCandidates = tables.filter((t) =>
			parentConcepts.every((c) => t.names.includes(c.name)),
		);
		let inputData =
			inputDataCandidates.length > 0 ? inputDataCandidates[0] : tables[0];
		...
		cardBottomComponents = [
			...
			<DisambiguationDialog
				...
				handleUpdate={(code, desc, closeDialog) => {
		...
		const checkDerivedConceptDiff = () => {
			...
			return (
				...
				conceptTransform.parentIDs.toString() !=
					transformParentIDs.toString()
			);
		};
		...
		if (concept.source == "derived") {
			...
			if (
				transformResult.filter((entry) => entry[1] == undefined)
					.length > 0
			) {
				...
				<Box
					...
					<ButtonGroup
						...
						<Button
							size="small"
							variant={
								checkDerivedConceptDiff()
									? "contained"
									: "outlined"
							}
							disabled={
								saveDisabledMsg.length > 0 ||
								checkDerivedConceptDiff() == false
							}
							onClick={() => {
								let tmpConcept = duplicateField(concept);
								tmpConcept.name = name;
...
export const CodexDialogBox: FC<CodexDialogBoxProps> = function ({
	...
	let formulateButton = (
		<Tooltip title="Derived the new concept">
			<IconButton
				size={size}
				...
				onClick={() => {
					...
					console.log(
						`[fyi] just sent request "${description}" at ${requestTimeStamp}`,
					);
					...
					fetch(getUrls().DERIVE_CONCEPT_URL, {
						...message,
	...
	let textBox = (
		<Box key="interaction-comp" width="100%" sx={{ display: "flex" }}>
			<TextField
				size="small"
				sx={{ fontSize: 12 }}
				color="primary"
