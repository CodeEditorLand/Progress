...
export let ChartElementFC: FC<{
	...
	boxWidth?: number;
	boxHeight?: number;
}> = function ({ chart, tableRows, boxWidth, boxHeight }) {
...
export const EncodingShelfThread: FC<EncodingShelfThreadProps> = function ({
	...
	let activeThreadChartId = useSelector(
		(state: DataFormulatorState) => state.activeThreadChartId,
	);
	...
	if (activeThreadChart != undefined) {
		let t = tables.find(
			(t) => t.id == (activeThreadChart as Chart).tableRef,
		) as DictTable;
		activeTableThread = getTriggers(t, tables).map((tr) => tr.tableId);
		activeTableThread = [
			...activeTableThread,
			(activeThreadChart as Chart).tableRef,
		];
	}
	...
	let reFormulate = (trigger: Trigger) => {
		...
		let triggerTable = tables.find(
			(t) => t.id == trigger.tableId,
		) as DictTable;

		let baseTables = trigger.sourceTableIds
		...
		let triggerChart = charts.find(
			(c) => c.id == trigger.chartRef,
		) as Chart;
		...
		let activeFields = Object.values(triggerChart.encodingMap)
			.map((enc) => enc.fieldID)
			.filter(
				(fieldId) =>
					fieldId &&
					conceptShelfItems.map((f) => f.id).includes(fieldId),
			)
			.map(
				(fieldId) =>
					conceptShelfItems.find((f) => f.id == fieldId) as FieldItem,
			);
		let activeBaseFields = activeFields
			.map((f) =>
				f.source == "derived"
					? (f.transform as ConceptTransformation).parentIDs
					: [f.id],
			)
			.flat()
			.map(
		...
		let messageBody = JSON.stringify({
			token: token,
			mode,
			input_tables: baseTables.map((t) => {
				return { name: t.id.replace(/\.[^/.]+$/, ""), rows: t.rows };
			}),
			new_fields: activeBaseFields.map((f) => {
				return { name: f.name };
			}),
			extra_prompt: prompt,
			model: activeModel,
				...
				if (data.results.length > 0) {
					if (data["token"] == token) {
						...
						if (candidates.length == 0) {
							...
							dispatch(
								dfActions.overrideDerivedTables(candidateTable),
							);
							dispatch(fetchFieldSemanticType(candidateTable));
							dispatch(fetchCodeExpl(candidateTable));

							if (
								triggerChart.chartType != "Auto" &&
								charts.find(
									(c) => c.tableRef == overrideTableId,
								)
							) {
								let cId = [
									...charts.filter(
										(c) => c.intermediate == undefined,
									),
									...charts,
								].find(
									(c) => c.tableRef == overrideTableId,
								...
								dispatch(
									dfActions.setFocusedChart(newChart.id),
								);
							}
							...
							dispatch(dfActions.clearUnReferencedTables());
							dispatch(
								dfActions.clearUnReferencedCustomConcepts(),
							);
							dispatch(
								dfActions.setFocusedTable(candidateTable.id),
							);
		...
		<div key={`${tableId}-table-list-item`} className="table-list-item">
			<Button
				...
				<Stack
					...
					<Typography sx={{ fontSize: "12px" }}>{tableId}</Typography>
				</Stack>
			</Button>
		</div>
	));
	...
	let instructionCards = triggers.map((trigger, i) => {
		let extractActiveFields = (t: Trigger) => {
			let encodingMap = (charts.find((c) => c.id == t.chartRef) as Chart)
				.encodingMap;
	...
	if (chart.intermediate) {
		let activeThreadChart = (
			activeThreadChartId
				? charts.find((c) => c.id == activeThreadChartId)
				: charts.find(
						(c) =>
							c.intermediate == undefined &&
							c.tableRef ==
								activeTableThread[activeTableThread.length - 1],
					)
		) as Chart;
		let endChartId = activeThreadChart.id;

		let activeEndChartTable = tables.find(
			(t) => t.id == activeThreadChart.tableRef,
		) as DictTable;
		let endChartCard = (
			<Card
				variant="outlined"
				...
				onClick={() => {
					...
					dispatch(
						dfActions.setFocusedTable(activeThreadChart.tableRef),
					);
				}}
				sx={{
					padding: "2px 0 2px 0",
				...
				<ChartElementFC
					chart={activeThreadChart}
					tableRows={activeEndChartTable.rows}
					boxWidth={200}
					boxHeight={160}
				/>
			</Card>
		...
		let postInstructEndPoint = activeTableThread.findIndex(
				...
				<Box
					...
					{interleaveArrays(
						[
							...
							...instructionCards.slice(
								cutIndex + 1,
								postInstructEndPoint,
							),
						],
						tableList.slice(cutIndex + 1, postInstructEndPoint + 1),
						spaceElement,
					)}
