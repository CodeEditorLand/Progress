...
export function MessageSnackbar() {
	const challenges = useSelector(
		(state: DataFormulatorState) => state.activeChallenges,
	);
	const messages = useSelector(
		(state: DataFormulatorState) => state.messages,
	);
	...
	React.useEffect(() => {
		if (displayedMessageIdx < messages.length) {
			...
			dispatch(
				dfActions.setDisplayedMessageIndex(displayedMessageIdx + 1),
			);
		}
	}, [messages]);
	...
	let timestamp =
		message == undefined
			? ""
			: new Date((message as Message).timestamp).toLocaleString("en-US", {
					timeZone,
					hour: "2-digit",
					minute: "2-digit",
					second: "2-digit",
				...
				<IconButton
					...
					sx={{
						...
						"@keyframes glow": {
							from: {
								boxShadow:
									"0 0 5px #fff, 0 0 10px #fff, 0 0 15px #ed6c02",
							},
							to: {
								boxShadow:
									"0 0 10px #fff, 0 0 20px #fff, 0 0 30px #ed6c02",
							},
						},
					}}
					...
					<Paper
						...
						<Box
							...
							<Typography
								...
								Visualization challenges for dataset{" "}
								<Box
									component="span"
									sx={{
										fontWeight: "bold",
