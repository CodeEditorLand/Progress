...
export const DisambiguationDialog: FC<DisambiguationDialogProps> =
	function DisambiguationDialog({
		...
		useEffect(() => {
			setCodeList(codeCandidates);
		}, [codeCandidates]);
		useEffect(() => {
			setDescription(transformDesc);
		}, [transformDesc]);
		...
		let parentConcepts = parentIDs.map(
			(id) => conceptShelfItems.find((f) => f.id == id) as FieldItem,
		);
				...
				<DialogContent sx={{ overflowX: "hidden" }} dividers>
					...
					<Box
						...
						{codeList.map((code, idx) => {
							...
							let endIdx =
								displayPage * displayPageSize + displayPageSize;
							if (endIdx > codeOutputs[idx].length) {
								endIdx = codeOutputs[idx].length;
								startIdx = Math.max(
									codeOutputs[idx].length - displayPageSize,
									0,
								);
							}
							...
							let codeOutputSamples = codeOutputs[idx].slice(
								startIdx,
								endIdx,
							);
							let highlightedRows = codeOutputSamples.map(
								(pair, k) =>
									pair[1] !=
									codeOutputs[selectionIdx].slice(
										startIdx,
										endIdx,
									)[k][1],
							);
...
export let simpleTableView = (
	exampleOutputs: [any[], any][],
