...
export let getDataTable = (
	...
	let activeBaseFields = conceptShelfItems.filter((field) => {
		return activeFields.some((f) =>
			findBaseFields(f, conceptShelfItems)
				.flat()
				.map((x) => x.id)
				.includes(field.id),
		);
	...
	if (confirmedTableCandidates.length > 0) {
		...
		return tables
			...
			.sort(
				(a, b) =>
					activeFields.filter((f) => a.names.includes(f.name))
						.length -
...
export let CodeBox: FC<{ code: string; language: string }> = function CodeBox({
	code,
	language,
}) {
	useEffect(() => {
...
export const chartAvailabilityCheck = (
	...
	let unavailable =
		...
		Object.entries(encodingMap).filter(
			(entry) => entry[1].fieldID != undefined,
		).length == 0;
	return [!unavailable, unfilledFields];
};

const BaseChartCreationMenu: FC<{ tableId: string; buttonElement: any }> =
	function BaseChartCreationMenu({ tableId, buttonElement, ...other }) {
...
export const ChartEditorFC: FC<{
	...
	const [candidatesViewAnchorEl, setCandidatesViewAnchorEl] =
	...
	let resultTable = tables.find(
		(t) => t.id == focusedChart.intermediate?.resultTableId,
	);
	...
	let toDeriveFields = derivedFields
		.filter((f) => f.name != "")
		.filter((f) =>
			findBaseFields(f, conceptShelfItems).every((f2) =>
				table.names.includes(f2.name),
			),
		);
	let focusedExtTable = baseTableToExtTable(
		JSON.parse(JSON.stringify(table.rows)),
		toDeriveFields,
		conceptShelfItems,
	);
			...
			.then(function (result) {
				...
				if (result.view.container()?.getElementsByTagName("svg")) {
					...
					if (comp) {
						...
						comp?.setAttribute(
							"style",
							`width: ${width * scaleFactor}px; height: ${height * scaleFactor}px;`,
						);
					}
				}
	...
	let focusedChartElement = createChartElement(
		focusedChart,
		focusedExtTable,
		`focused-element-${focusedChart.id}`,
	);
	let arrowCard = <></>;
	...
	let createNewChartButton = (
		<ChartCreationMenu
			tableId={focusedChart.tableRef}
			buttonElement={
	...
	let derivedTableItems =
		resultTable?.derive || table.derive
			? [
					<Tooltip
						title={`${codeViewOpen ? "hide" : "view"} transformation code`}
						key="code-view-btn-tooltip">
						<IconButton
							color="primary"
							size="small"
							sx={{
					...
					<Tooltip
						title={`${codeExplViewOpen ? "hide" : "view"} transformation explanation`}
						key="code-expl-view-btn-tooltip">
						<IconButton
							color="primary"
							size="small"
							sx={{
							...
							onClick={() => {
								setCodeExplViewOpen(!codeExplViewOpen);
							}}>
							<TextSnippetIcon />
						</IconButton>
					</Tooltip>,
					...
					/>,
					<Tooltip
						title="view agent dialog"
						key="view-chat-history-btn-tooltip">
						<IconButton
							color="primary"
							size="small"
							sx={{ textTransform: "none" }}
					...
					<ChatDialog
						...
						code={transformCode}
						dialog={
							resultTable?.derive?.dialog ||
							(table.derive?.dialog as any[])
						}
					/>,
				]
			: [];
	...
	let chartActionButtons = [
		...
		focusedChart.intermediate == undefined
	...
	let chartActionItems = chartUnavailable ? (
		<Box
			...
			{synthesisRunning || Boolean(candidatesViewAnchorEl) ? (
				""
			) : (
				<Typography
					component="div"
					...
					{Object.entries(focusedChart.encodingMap).filter(
						(entry) => entry[1].fieldID != undefined,
					).length == 0 ? (
						<Typography component="span" fontSize="inherit">
							{focusedChart.chartType == "Table"
								...
								: focusedChart.chartType == "Auto"
									? "Say something to get chart recommendations!"
									: "To create a chart, put data fields to chart builder."}
						</Typography>
					) : (
						<Typography component="span" fontSize="inherit">
							Once you provided all fields, "formulate" to create
							the visualization.
						</Typography>
					)}
			...
			overrides={{
				...
				p: {
					props: {
						style: {
							fontFamily:
								"Arial, Roboto, Helvetica Neue, sans-serif",
							fontWeight: 400,
							fontSize: 12,
							lineHeight: 2,
							margin: 0,
		...
		focusedComponent = [
			...
			<Box
				...
				<DerivedDataDialog
					...
					open={
						Boolean(candidatesViewAnchorEl) && candidates.length > 0
					}
					handleCloseDialog={() => {
						setCandidatesViewAnchorEl(null);
					}}
					handleSelection={(selectionIdx: number) => {
						...
						dispatch(
							dfActions.replaceTable({
								...
								table: candidates[selectionIdx],
							}),
						);
						dispatch(
							dfActions.setFocusedTable(
								candidates[selectionIdx].id,
							),
						);
						setCandidatesViewAnchorEl(null);
					}}
							...
							<Box
								...
								<Card
									...
									<CardContent
										...
										<Typography
											sx={{ fontSize: 14, margin: 1 }}
											gutterBottom>
											Data transformation code (
											{table.derive?.source} → {table.id})
										</Typography>
										<Box
											sx={{
												display: "flex",
		...
		<Stack
			...
			<Tooltip title="zoom out">
				<IconButton
					color="primary"
					size="small"
					disabled={scaleFactor <= scaleMin}
					onClick={() => {
						dispatch(
							dfActions.updateChartScaleFactor({
								chartId: focusedChart.id,
...
export const VisualizationViewFC: FC<VisPanelProps> =
	function VisualizationView({}) {
		...
		let visViewMode = useSelector(
			(state: DataFormulatorState) => state.visViewMode,
		);

		let [cachedCandidates, setCachedCandidates] = useState<
			{ chartId: string; tables: DictTable[] }[]
		>([]);
		let handleUpdateCandidates = (
			chartId: string,
			candidates: DictTable[],
		) => {
			setCachedCandidates([
				{ chartId, tables: candidates },
				...cachedCandidates.filter((l) => l.chartId != chartId),
			]);
		};
		...
		if (
			...
			charts.find((c) => c.id == focusedChartId)?.chartType == "?"
		) {
			let chartSelectionBox = (
				<Box
					...
					{Object.entries(CHART_TEMPLATES)
						.map(([cls, templates]) => templates)
						.flat()
						.filter((t) => t.template["name"] != "?")
						.map((t) => (
							<Button
								key={`${t.chart}-btn`}
								sx={{
			...
			return <Box sx={{ margin: "auto" }}>{chartSelectionBox}</Box>;
		}
		...
		let chartEditor = (
			<ChartEditorFC
				key={focusedChartId}
				cachedCandidates={
					cachedCandidates.find((l) => l.chartId == focusedChartId)
						?.tables || []
				}
		...
		if (visViewMode == "gallery") {
			let chartElements = charts
				.filter((c) => !c.intermediate)
				.map((chart, index) => {
					...
					let extTable = baseTableToExtTable(
						JSON.parse(JSON.stringify(table.rows)),
						toDeriveFields,
						conceptShelfItems,
					);
					...
					if (chart.chartType == "Table") {
						return (
							<Box
								key={`animateOnChange-${index}`}
								className="vega-thumbnail-box"
								onClick={setIndexFunc}
								sx={{
									position: "relative",
									backgroundColor: chart.saved
										? "rgba(255,215,0,0.05)"
										: "white",
									border: chart.saved
										? "2px solid gold"
					...
					let [available, unfilledFields] = chartAvailabilityCheck(
						chart.encodingMap,
						conceptShelfItems,
						extTable,
					);
					if (!available) {
						return (
							<Box
								...
								key="skeleton"
								onClick={setIndexFunc}>
								{generateChartSkeleton(chartTemplate?.icon)}
							</Box>
						);
					}
			...
			finalView = (
				<Box
					...
					<Box
						className="visualization-carousel"
						sx={{ display: "contents" }}>
						{chartEditor}
					</Box>
				</Box>
