...
export const getDomains = (field: FieldItem, tables: DictTable[]): any[][] => {
	let domains = tables
		.filter((t) => t.names.includes(field.name))
	...
	domains = domains.filter((d, i) => {
		return !domains
			...
			.some(
				(prevD) =>
					JSON.stringify(prevD.slice().sort()) ==
					JSON.stringify(d.slice().sort()),
			);
	});
...
export const deriveTransformExamplesV2 = (
	code: string,
	parentIDs: string[],
	...
	let examplesFromEachParent =
		inputListLength > 0
			? parentDomains.map((l) => l.slice(0, inputListLength))
			: parentDomains;

	let maxValueLength =
		examplesFromEachParent.length > 0
			? Math.min(...examplesFromEachParent.map((l) => l.length))
			: 0;

	let inputTupleList = [...Array(maxValueLength).keys()].map((i) =>
		examplesFromEachParent.map((exampleList) => exampleList[i]),
	);

	let argList = inputTupleList;

	if (inputRequiresColumnList) {
		argList = inputTupleList.map((args, i) => [
			...args,
			i,
			...parentDomains,
		]);
...
export const processCodeCandidates = (
	rawCodeList: string[],
	parentIDs: string[],
	conceptShelfItems: FieldItem[],
	tables: DictTable[],
	...
	let codeOutputSignatures = tempCodeList
		.map((codeStr) => {
			let result = [
				codeStr,
		...
		.map(
			([codeStr, ioPairs]) =>
				[
					codeStr,
					ioPairs.map((ioPair) => `${ioPair[1]}`).join("---"),
				] as [string, string],
		);
...
export const findBaseFields = (
	field: FieldItem,
	conceptShelfItems: FieldItem[],
): FieldItem[] => {
	if (field.source == "derived") {
		return (field.transform as ConceptTransformation).parentIDs
			.map((parentID) =>
				findBaseFields(
...
export const groupConceptItems = (conceptShelfItems: FieldItem[]) => {
	...
	return conceptShelfItems.map((f) => {
		...
		if (f.source == "original") {
			...
			group =
				findBaseFields(f, conceptShelfItems)[0].tableRef ||
				"custom concepts";
		}
