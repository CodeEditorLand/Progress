...
const SelectableCell = createSelectable<SelectableCellProps>(
	(props: TSelectableItemProps & SelectableCellProps) => {
		let {
			selectableRef,
			selected,
			isSelected,
...
export const SelectableDataGrid: React.FC<SelectableDataGridProps> = ({
	...
	const [footerActionExpand, setFooterActionExpand] =
	...
	const onClickCell = (event: any, rowIndex: number, colIndex: number) => {
		...
		selectedCells.splice(
			_.sortedIndex(selectedCells, [rowIndex, colIndex]),
			0,
			[rowIndex, colIndex],
		);
		setSelectedCells([...selectedCells]);
	...
	const TableComponents = {
		...
		Table: Table,
		TableHead: (props: any) => (
			<TableHead {...props} className="table-header-container" />
		),
		TableRow: (props: any) => {
		...
		<Box sx={{ display: "flex" }}>
			...
			<Tooltip title="Infer Data Type">
				<IconButton
					...
					onClick={() => {
						console.log(
							`[fyi] just sent request to process load data`,
						);
						...
						fetch(getUrls().SERVER_PROCESS_DATA_ON_LOAD, {
							...message,
