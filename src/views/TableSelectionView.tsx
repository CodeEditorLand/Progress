				...
				{tableChallenges.map((tc, i) => {
					...
					let challengeView = (
						<Box sx={{ margin: "6px 0px" }}>
							<Typography
								variant="subtitle2"
								sx={{ marginLeft: "6px", fontSize: 12 }}>
								Try these data visualization challenges with
								this dataset:
							</Typography>
							{tc.challenges.map((c, j) => (
								<Box
									key={j}
									sx={{
										display: "flex",
...
export const TableSelectionDialog: React.FC<{ buttonElement: any }> =
	function TableSelectionDialog({ buttonElement }) {
		...
		React.useEffect(() => {
			...
			fetch(`${getUrls().VEGA_DATASET_LIST}`)
				...
				.then((result) => {
					let tableChallenges: TableChallenges[] = result
						.map((info: any) => {
							let table = createTableFromFromObjectArray(
								info["name"],
								JSON.parse(info["snapshot"]),
							);
							return {
								table: table,
						...
						.filter(
							(t: TableChallenges | undefined) => t != undefined,
						);
			...
			<>
				...
				<Dialog
					...
					<DialogContent
						...
						dividers>
						<TableSelectionView
							tableChallenges={datasetPreviews}
							hideRowNum
							handleDeleteTable={undefined}
							handleSelectTable={(tableChallenges) => {
								...
								console.log(
									`${getUrls().VEGA_DATASET_REQUEST_PREFIX}${tableChallenges.table.id}`,
								);
								fetch(
									`${getUrls().VEGA_DATASET_REQUEST_PREFIX}${tableChallenges.table.id}`,
								)
									...
									.then((text) => {
										let fullTable =
											createTableFromFromObjectArray(
												tableChallenges.table.id,
												JSON.parse(text),
											);
										if (fullTable) {
											dispatch(
												dfActions.loadTable(fullTable),
											);
											dispatch(
												fetchFieldSemanticType(
													fullTable,
											...
											dispatch(
												dfActions.addChallenges({
													tableId:
														tableChallenges.table
															.id,
													challenges:
														tableChallenges.challenges,
												}),
											);
										} else {
											throw "";
			...
			for (let file of files) {
				...
				if (
					...
					reader.onload = (e) => {
						...
						if (arrayBuffer) {
							let tables = loadBinaryDataWrapper(
								uniqueName,
								arrayBuffer,
							);
...
export const TableCopyDialogV2: React.FC<TableCopyDialogProps> = ({
	...
	const [cleaningInProgress, setCleaningInProgress] =
		useState<boolean>(false);
	const [cleanTableContent, setCleanTableContent] = useState<
		{ content: string; reason: string; mode: string } | undefined
	>(undefined);
			...
			.catch((error) => {
				...
				dispatch(
					dfActions.addMessages({
						...
						"type": "error",
						"value":
							"unable to perform clean data due to server issue.",
					}),
				);
			});
	};
	...
	let renderLines = (str: string) => <span style={{}}>{str}</span>;
		...
		<Dialog
					...
					{viewTable ? (
						...
						<TextField
							...
							sx={{
								...
								"& .MuiInputBase-input": {
									fontSize:
										tableContent.length > 1000 ? 12 : 14,
									lineHeight: 1.2,
								},
							}}
							id="upload content"
						...
						<Box
							...
							<Box sx={{ marginTop: 1, position: "relative" }}>
								...
								{validator.isURL(tableContent) ||
								validator.isDataURI(tableContent) ? (
									<img
										style={{
										...
										src={DOMPurify.sanitize(tableContent)}
										alt="the image is corrupted, please try again."
									/>
								) : (
							...
							<TextField
								...
								value={imageCleaningInstr}
								onChange={(event) => {
									setImageCleaningInstr(event.target.value);
								}}
								variant="standard"
			...
			<DialogActions>
				{cleanTableContent != undefined ? (
					...
					<Button
						disabled={tableContent.trim() == "" || loadFromURL}
						variant={cleaningInProgress ? "outlined" : "contained"}
						color="primary"
						size="small"
				...
				<Button
					disabled={cleanTableContent != undefined}
					variant="contained"
					size="small"
					onClick={() => {
						setDialogOpen(false);
				...
				<Button
					disabled={
						cleanTableContent?.content == undefined &&
						(tableContentType != "text" ||
							tableContent.trim() == "")
					}
					variant="contained"
					...
					onClick={() => {
						setDialogOpen(false);
						handleSubmitContent(
							cleanTableContent?.content || tableContent,
						);
					}}>
					{"upload"}
				</Button>
