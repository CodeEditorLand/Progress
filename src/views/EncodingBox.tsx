...
export const LittleConceptCard: FC<LittleConceptCardProps> =
	function LittleConceptCard({ channel, field, encoding, handleUnbind }) {
...
export const EncodingBox: FC<EncodingBoxProps> = function EncodingBox({
	...
	let [autoSortResult, setAutoSortResult] = useState<
		{ values: any[]; reason: string } | undefined
	>(field?.levels);
	...
	let domainItems =
		field?.source == "custom" || field?.source == "original"
			? getDomains(field as FieldItem, tables)[0]
			: [];
	...
	let autoSortEnabled =
		field && field?.type == "string" && domainItems.length < 200;
			...
			.catch((error) => {
				...
				dispatch(
					dfActions.addMessages({
						...
						"type": "error",
						"value":
							"unable to perform auto-sort due to server issue.",
					}),
				);
			});
	};

	let sortByOptions = [radioLabel("default", "default", `sort-by-default`)];
	// TODO: check sort options
	if (channel == "x" && (field?.type == "string" || field?.type == "auto")) {
		sortByOptions.push(
			radioLabel("y values", "y", `sort-x-by-y-ascending`, 90),
		);
	}
	if (channel == "y" && (field?.type == "string" || field?.type == "auto")) {
		sortByOptions.push(
			radioLabel("x values", "x", `sort-y-by-x-ascending`, 90),
		);
	}
	...
	let binDisplay = encoding.bin ? (
		<Chip
			key="bin-display"
			className="encoding-prop-chip"
			color="default"
			label={"bin"} //deleteIcon={<RemoveIcon />}
			sx={{ backgroundColor: optBackgroundColor }}
			size="small"
			onDelete={() => updateEncProp("bin", false)}
		/>
	) : (
		""
	);
	...
	let handleSelectOption = (option: string) => {
		if (conceptShelfItems.map((f) => f.name).includes(option)) {
			...
			updateEncProp(
				"fieldID",
				(conceptShelfItems.find((f) => f.name == option) as FieldItem)
					.id,
			);
		} else {
			if (option == "...") {
	...
	let createConceptInputBox = (
		<Autocomplete
			...
			groupBy={(option) => {
				let groupItem = conceptGroups.find(
			...
			renderOption={(props, option) => {
				let renderOption =
					conceptShelfItems.map((f) => f.name).includes(option) ||
					option == "..."
						? option
						: `"${option}"`;
	...
	const encContent =
		field == undefined
			...
			: [normalizedDisplay, aggregateDisplay, binDisplay, fieldComponent];
