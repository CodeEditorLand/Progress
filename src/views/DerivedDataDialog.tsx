...
import { Chart, createDictTable, DictTable } from "../components/ComponentType";
...
export const DerivedDataDialog: FC<DerivedDataDialogProps> =
	function DerivedDataDialog({
		...
		let direction =
			candidateTables.length > 1 ? "horizontal" : "horizontal";
				...
				{candidateTables.map((table, idx) => {
					...
					const element = (
						<Box
							...
							onClick={() => setSelectionIdx(idx)}></Box>
					);
							...
							if (
								...
								if (comp) {
									...
									if (width > 240 || height > 180) {
										...
										comp?.setAttribute(
											"style",
											`max-width: 240px; max-height: 180px; width: ${Math.round(fixedWidth)}px; height: ${Math.round(fixedWidth / ratio)}px; `,
										);
									}
								} else {
									console.log(
										"THUMB: Could not get Canvas HTML5 element",
									);
								}
							}
						})
						...
						<Card
							...
							<CardContent
								...
								<FormControlLabel
									...
									value={idx}
									control={
										<Radio checked={selectionIdx == idx} />
									}
									label={
										<Typography
											color={
												idx == selectionIdx
													? "primary"
													: "inherit"
											}>{`candidate-${idx + 1} (${candidateTables[idx].id})`}</Typography>
									}
								/>
								<Box
									sx={{
								...
								<Button
									...
									<Typography
										...
										sx={{
											margin: "0px 2px",
											padding: "0px 2px",
										}}>{`candidate ${selectionIdx + 1} (${candidateTables[selectionIdx].id})`}</Typography>{" "}
									as the result
								</Button>
							</ButtonGroup>
						</Box>
