		...
		<>
			...
			<Dialog onClose={() => setOpen(false)} open={open}>
				...
				<DialogContent>
					<DialogContentText>
						<Typography>
							All unexported content (charts, derived data,
							concepts) will be lost upon reset.
						</Typography>
					</DialogContentText>
				</DialogContent>
				<DialogActions>
							...
							<Box
								...
								<Box sx={{ flex: 1 }}>
									...
									<Typography
										...
										Maximum time allowed for the formulation
										process before timing out.
									</Typography>
									<Typography
										variant="caption"
										color="text.secondary"
										sx={{ mt: 1, display: "block" }}>
										Smaller values (&lt;30s) make the model
										fails fast thus providing a smoother UI
										experience. Increase this value for slow
										models.
									</Typography>
								</Box>
							</Box>
							<Box
								...
								<Box sx={{ flex: 1 }}>
									...
									<Typography
										...
										color="text.secondary"
										sx={{ mt: 1, display: "block" }}>
										Maximum number of times the LLM will
										attempt to repair code if generated code
										fails to execute (recommended = 1).
									</Typography>
									<Typography
										variant="caption"
										color="text.secondary"
										sx={{ mt: 1, display: "block" }}>
										Higher values might slightly increase
										the chance of success but can crash the
										backend. Repair time is as part of the
										formulate timeout.
									</Typography>
								</Box>
							</Box>
		...
		fetch("/.auth/me")
			...
			.then(function (result) {
				if (Array.isArray(result) && result.length > 0) {
					...
					let userInfo = {
						name:
							authInfo["user_claims"].find(
								(item: any) => item.typ == "name",
							)?.val || "",
			...
			errorElement: (
				<Box sx={{ width: "100%", height: "100%", display: "flex" }}>
					<Typography color="gray" sx={{ margin: "150px auto" }}>
						...
						<Link href="/">refresh the session</Link>. If the
						problem still exists, click close session.
					</Typography>
				</Box>
			),
		},
