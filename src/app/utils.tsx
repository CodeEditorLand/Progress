...
export function runCodeOnInputListsInVM(
	...
	"use strict";
	let ioPairs: [any[], any][] = inputTupleList.map((args) => [
		args,
		undefined,
	]);
	if (mode == "safer") {
		try {
			...
			if (code != "") {
				...
				ioPairs = inputTupleList.map((args, i) => [
					args,
					context.outputs[i],
				]);
				return ioPairs;
...
export function baseTableToExtTable(
	table: any[],
	derivedFields: FieldItem[],
	allFields: FieldItem[],
) {
	...
	let availableBaseFields = allFields.filter((f) =>
		Object.keys(table[0]).includes(f.name),
	);
	let extDerivedFields = [...derivedFields];
	while (true) {
		let unresolvedDerivedParentID = extDerivedFields
			.map((field) => {
				let parentIDs = (field.transform as ConceptTransformation)
					.parentIDs;
				return allFields
					.filter(
						(f) =>
							parentIDs.includes(f.id) && f.source == "derived",
					)
					.filter(
						(f) =>
							!extDerivedFields.map((f) => f.id).includes(f.id),
					)
					.map((f) => f.id);
			})
			.flat();
		...
		if (unresolvedDerivedParentID.length == 0) {
			...
			extDerivedFields = [
				...extDerivedFields,
				...allFields.filter((f) =>
					unresolvedDerivedParentID.includes(f.id),
				),
			];
		}
	}
	...
	while (!extDerivedFields.every((f) => derivedColID2Cols.has(f.id))) {
		let readyFields = extDerivedFields
			.filter((f) => !derivedColID2Cols.has(f.id))
			.filter((f) =>
				(f.transform as ConceptTransformation).parentIDs.every(
					(parentID) =>
						[
							...derivedColID2Cols.keys(),
							...availableBaseFields.map((f) => f.id),
						].includes(parentID),
				),
			);
		...
		let newlyDerivedCols: [string, string[], any[]][] = readyFields.map(
			(field) => {
				...
				let parentNames = (
					...
					(parentID) =>
						(allFields.find((f) => f.id == parentID) as FieldItem)
							.name,
				);
				let baseCols = (
					field.transform as ConceptTransformation
				).parentIDs.map((parentID) => {
					let baseField = availableBaseFields.find(
						(f) => f.id == parentID,
					);
					if (baseField != undefined) {
						return table.map(
							(row) => row[(baseField as FieldItem).name],
						);
					} else {
						return (
							derivedColID2Cols.get(parentID) as [
								string,
								string[],
								any[],
							]
				...
				let jsCode = ts.transpile(
					(field.transform as ConceptTransformation).code as string,
				);
				let func = eval(jsCode);
	...
	while (true) {
		let missingCols = derivedCols.filter(
			(c) => !orderedNames.includes(c[0]),
		);
		...
		for (let [name, parentNames, vals] of missingCols) {
			...
			let lastParent = (parentNames as string[]).sort(
				(n1, n2) => orderedNames.indexOf(n2) - orderedNames.indexOf(n1),
			)[0];
...
export const assembleVegaChart = (
	...
	for (const [channel, encoding] of Object.entries(encodingMap)) {
		...
		if (field) {
			...
			if (encodingObj["type"] == "nominal" && channel == "color") {
				// special case, unify
				let actualDomain = [
					...new Set(workingTable.map((r) => r[field.name])),
				];
				if (
					actualDomain.every((v) => field.domain.includes(v)) &&
					field.domain.length > actualDomain.length
				) {
		...
		if (encoding.sortBy || encoding.sortOrder) {
			...
			if (encoding.sortBy == undefined || encoding.sortBy == "default") {
				...
				encodingObj["sort"] =
					`${sortOrder == "ascending" ? "" : "-"}${encoding.sortBy}`;
			} else {
				try {
		...
		if (
			Object.keys(encodingObj).length != 0 &&
			chartTemplate.paths[channel]
		) {
	...
	if (
		vgObj.encoding?.column != undefined &&
		vgObj.encoding?.row == undefined
	) {
	...
	values = values.map((r: any) => {
		...
		let temporalKeys = keys.filter((k: string) =>
			conceptShelfItems.some(
				(concept) =>
					concept.name == k &&
					(concept.type == "date" || concept.semanticType == "Year"),
			),
		);
	...
	for (const channel of ["x", "y", "column", "row", "xOffset"]) {
		...
		if (encoding?.type === "nominal") {
			...
			const uniqueValues = [
				...new Set(values.map((r: any) => r[fieldName])),
			];
...
export const adaptChart = (chart: Chart, targetTemplate: ChartTemplate) => {
	let discardedChannels = Object.entries(chart.encodingMap).filter(
		([ch, enc]) => {
			return (
				!targetTemplate.channels.includes(ch) &&
				enc.fieldID != undefined
			);
		},
	);
...
export const resolveChartFields = (
	...
	let targetFieldIds: string[] = targetFieldNames
		.map((name) => currentConcepts.find((c) => c.name == name)?.id)
	...
	let ocupiedChannels = chartChannels.filter((ch) => {
		...
		return (
			fieldId != undefined &&
			table.names.includes(
				currentConcepts.find((c) => c.id == fieldId)?.name || "",
			)
		);
	});
	let ocupiedFieldIds = ocupiedChannels.map(
		(ch) => chart.encodingMap[ch as keyof EncodingMap].fieldID,
	);

	let newAdditionFieldIds = targetFieldIds.filter(
		(fid) => !ocupiedFieldIds.includes(fid),
	);
	let channelsToUpdate = [
		...chartChannels.filter((ch) => !ocupiedChannels.includes(ch)),
	];

	for (
		let i = 0;
		i < Math.max(newAdditionFieldIds.length, channelsToUpdate.length);
		i++
	) {
		chart.encodingMap[channelsToUpdate[i] as keyof EncodingMap].fieldID =
			newAdditionFieldIds[i];
	}
