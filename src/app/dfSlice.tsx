...
export const generateFreshChart = (
	...
	return {
		...
		encodingMap: Object.assign(
			{},
			...getChartChannels(realChartType).map((channel) => ({
...
let getUnrefedDerivedTableIds = (state: DataFormulatorState) => {
	...
	let chartRefedTables = state.charts.map((chart) =>
		getDataTable(
			chart,
			state.tables,
			state.charts,
	...
	let triggerRefedTableIds = chartRefedTables
		.filter((t) => t.derive != undefined)
		.map((t) => t.derive?.trigger as Trigger);

	let allRefedTableIds = [
		...chartRefedTables.map((t) => t.id),
		...triggerRefedTableIds,
	];

	// TODO: also need to consider concept shelf reference??

	return state.tables
		.filter((table) => table.derive && !allRefedTableIds.includes(table.id))
		.map((t) => t.id);
};
...
let deleteChartsRoutine = (state: DataFormulatorState, chartIds: string[]) => {
	...
	if (focusedChartId && chartIds.includes(focusedChartId)) {
		...
		state.focusedTableId = charts.find(
	...
	state.tables = state.tables.filter(
		(t) => !unrefedDerivedTableIds.includes(t.id),
	);
	...
	state.charts = state.charts.filter(
		(c) =>
			!(
				c.intermediate &&
				unrefedDerivedTableIds.includes(c.intermediate.resultTableId)
			),
	);
};
		...
		let message = {
			...
			body: JSON.stringify({
				...
				input_tables: derivedTable.derive?.source
					.map(
						(tId) =>
							state.tables.find((t) => t.id == tId) as DictTable,
					)
					.map((t) => {
						return { name: t.id, rows: t.rows };
					}),
...
export const dataFormulatorSlice = createSlice({
	...
	reducers: {
		...
		loadState: (state, action: PayloadAction<any>) => {
			...
			state.activeThreadChartId =
				savedState.activeThreadChartId || undefined;
		...
		deleteTable: (state, action: PayloadAction<string>) => {
			...
			state.conceptShelfItems = state.conceptShelfItems.filter(
				(f) =>
					!(
						f.tableRef == tableId ||
						findBaseFields(f, state.conceptShelfItems).some(
							(f2) => f2.tableRef == tableId,
						)
					),
			);
			...
			state.charts = state.charts.filter(
				(c) =>
					!(
						c.intermediate &&
						c.intermediate.resultTableId == tableId
					),
			);
		},
		addChallenges: (
		...
		updateChartScaleFactor: (
			state,
			action: PayloadAction<{ chartId: string; scaleFactor: number }>,
		) => {
		...
		updateChartType: (
			...
			state.charts = state.charts.map((chart) => {
				if (chart.id == chartId) {
					return adaptChart(
						chart,
						getChartTemplate(chartType) as ChartTemplate,
					);
				} else {
					return chart;
				}
		...
		replaceTable: (
			state,
			action: PayloadAction<{ chartId: string; table: DictTable }>,
		) => {
			let chartId = action.payload.chartId;
			let chart = state.charts.find((c) => c.id == chartId) as Chart;
			let table = action.payload.table;
			let currentTableRef = getDataTable(
				chart,
				state.tables,
				state.charts,
				state.conceptShelfItems,
			...
			if (
				...
				state.tables = [
					...state.tables.filter((t) => t.id != currentTableRef),
					table,
				];
			} else {
				state.tables = [...state.tables, table];
		...
		updateChartEncodingProp: (
			...
			if (chart) {
				...
				if (prop == "fieldID") {
					...
					let field = state.conceptShelfItems.find(
						(f) => f.id == value,
					);
					if (field?.levels) {
		...
		setFocusedChart: (state, action: PayloadAction<string | undefined>) => {
			...
			if (chart?.intermediate == undefined) {
				...
				let currentActiveThreadChart = state.charts.find(
					(c) => c.id == state.activeThreadChartId,
				);
				let activeThreadChartTable = state.tables.find(
					(t) => t.id == currentActiveThreadChart?.tableRef,
				);

				if (activeThreadChartTable) {
					let triggers = getTriggers(
						activeThreadChartTable,
						state.tables,
					);
					if (
						triggers
							.map((tg) => tg.tableId)
							.includes(chart?.intermediate?.resultTableId)
					) {
						let nextChart = state.charts.find(
							(c) =>
								c.intermediate == undefined &&
								c.tableRef ==
									chart?.intermediate?.resultTableId,
						);
						if (nextChart) {
		...
		setVisViewMode: (
			state,
			action: PayloadAction<"carousel" | "gallery">,
		) => {
		...
		changeChartRunningStatus: (
			...
			if (action.payload.status) {
				...
				state.chartSynthesisInProgress =
					state.chartSynthesisInProgress.filter(
		...
		builder
			.addCase(fetchFieldSemanticType.fulfilled, (state, action) => {
				...
				if (data["status"] == "ok" && data["result"].length > 0) {
					...
					state.conceptShelfItems = state.conceptShelfItems.map(
						(field) => {
							if (
								((field.source == "original" &&
									field.tableRef == tableId) ||
									field.source == "custom") &&
								Object.keys(typeMap).includes(field.name)
							) {
								field.semanticType =
									typeMap[field.name]["semantic_type"];
								field.type = typeMap[field.name][
									"type"
								] as Type;
								if (typeMap[field.name]["sort_order"]) {
									field.levels = {
										"values":
											typeMap[field.name]["sort_order"],
										"reason": "natural sort order",
									};
								}
								return field;
							} else {
			...
			.addCase(fetchCodeExpl.fulfilled, (state, action) => {
				...
				let derivedTable = state.tables.find(
					(t) => t.id == derivedTableId,
				);
