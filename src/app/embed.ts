		...
		switch (action.actionName) {
				...
				if (subscription.loadData) {
					...
					try {
						table = createTableFromFromObjectArray(
							loadDataAction.actionParams.tableName || "dataset",
							loadDataAction.actionParams.table,
						);
					} catch (error) {
						console.error(
							"ActionQueue: error creating table from message",
							error,
						);
					}
					if (table) {
						console.log(
							"ActionQueue: success creating table from message",
						);
			...
			case "setConfig": {
				if (subscription.setAppConfig) {
					...
					subscription.setAppConfig(
						setConfigAction.actionParams as AppConfig,
					);
				}
				break;
			}
